...
from seer_attn.prefill_sparse_perq.llama.modeling_llama_seerattn_training import SeerAttnLlamaForCausalLM
...
def tokenize_fn(tokenizer, tranining_max_length, example):
    # raw_encoding = tokenizer(tokenizer.eos_token.join(example["text"]), return_tensors="pt", truncation=False, padding=False)
    # raw_length = len(raw_encoding["input_ids"][0])
...
def add_seq_lens(tokenizer, example):
    seqlens = []
    ...
    #     idx = len(input_ids) - 1
    #     while idx >= 0 and input_ids[idx] == eos_token_id:
    #         idx -= 1
    #     seqlen = idx + 1
    # else:
    #     seqlen = len(input_ids)
        
    seqlens.append(len(example["input_ids"]))
    return {"seq_lengths": seqlens}
...
def train():
    ...
    if "qwen2" in model_args.model_name_or_path.lower() or "r1-distill-qwen" in model_args.model_name_or_path.lower():
        ...
        model_cls = SeerAttnLlamaForCausalLM
    else:  
        raise ValueError("Model not supported. Current only support qwen2 or qwen3 or llama model.")
    ...
    dataset = dataset.map(partial(add_seq_lens, tokenizer), batched=False, num_proc=128)


    print(dataset)
    example = next(iter(dataset))
    
    # print("example:", example["input_ids"])
    # print("example input_ids shape:", len(example["input_ids"]))
    print("example seq_lengths:", example["seq_lengths"])
