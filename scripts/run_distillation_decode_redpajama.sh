...
training_max_length=${TRAINING_MAX_LENGTH:-49152} #57344
lr=${LR:-1e-3}
...
output_dir=${OUTPUT_DIR:-"/mnt/output/distilled_model_gates"}
steps=${STEPS:-500}
bs=${BS:-16}
loss_slice_ratio=${LOSS_SLICE_RATIO:-0.0}
dataset_name=${DATASET_NAME:-"togethercomputer/RedPajama-Data-1T-Sample"}
base_model=${BASE_MODEL:-"meta-llama/Llama-3.1-8B-Instruct"}
base_name=$(basename "$base_model")
prefix=${PREFIX:-"redpajama"}
...
torchrun --nproc_per_node=$gpus --master_port=10003 distillation_decode_redpajama.py  \
